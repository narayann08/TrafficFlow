  ...
  const handlePredict = async () => {
    ...
    while (!success && attempts < maxAttempts) {
      try {
        ...
        success = true;
        fetchHistory(); // Update history table after successful prediction
      } catch (err: any) {
        ...
        try {
          const mlApiUrl = process.env.NEXT_PUBLIC_ML_API_URL || "http://127.0.0.1:5000";
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-20 pb-12 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
                        ...
                        <CardContent>
                          <div className="h-72">
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={getComparisonChartData()} barSize={50} margin={{ top: 20, right: 100, left: 100, bottom: 0 }}>
                                <XAxis
                                  dataKey="name"
                                  stroke="#94a3b8"
                                  fontSize={13}
          ...
          {/* History Section */}
          <div className="mt-16">
            <Card className="border-border/50 bg-card/80 shadow-lg backdrop-blur-sm border-t-blue-500/20">
              <CardHeader className="bg-blue-50/5 dark:bg-blue-950/20 border-b border-border/50 pb-6 rounded-t-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl text-foreground font-bold tracking-tight">Recent Predictions</CardTitle>
                    <CardDescription className="text-sm mt-1">History of traffic level predictions saved in the database.</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="pt-6">
                <div className="rounded-xl border border-border overflow-hidden shadow-sm bg-background/50">
                  <Table>
                    <TableHeader className="bg-muted/80 backdrop-blur-md">
                      <TableRow className="hover:bg-transparent">
                        <TableHead className="font-semibold text-foreground/80 text-left pl-6">Time</TableHead>
                        <TableHead className="font-semibold text-foreground/80 text-left">Day</TableHead>
                        <TableHead className="font-semibold text-foreground/80 text-left">Algorithm</TableHead>
                        <TableHead className="font-semibold text-foreground/80 text-center">Result (KNN)</TableHead>
                        <TableHead className="font-semibold text-foreground/80 text-center">Result (RF)</TableHead>
                        <TableHead className="font-semibold text-foreground/80 text-center">Predicted At</TableHead>
                        ...
                        paginatedHistory.map((record, idx) => (
                          <TableRow key={idx} className="hover:bg-muted/40 transition-colors duration-200">
                            ...
                            <TableCell className="text-left">
                              <Badge variant="secondary" className="uppercase font-mono text-[10px] tracking-wider bg-secondary/50">
                                {record.input?.algorithm}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-center">
                              {record.prediction?.knn ? (
                                <Badge variant="outline" className="shadow-sm font-medium" style={{ color: TRAFFIC_COLORS[record.prediction.knn], borderColor: TRAFFIC_COLORS[record.prediction.knn] + '50', backgroundColor: TRAFFIC_COLORS[record.prediction.knn] + '10' }}>
                                  {record.prediction.knn}
                                </Badge>
                              ) : record.prediction?.traffic_level && record.input?.algorithm === 'knn' ? (
                                <Badge variant="outline" className="shadow-sm font-medium" style={{ color: TRAFFIC_COLORS[record.prediction.traffic_level], borderColor: TRAFFIC_COLORS[record.prediction.traffic_level] + '50', backgroundColor: TRAFFIC_COLORS[record.prediction.traffic_level] + '10' }}>
                                  {record.prediction.traffic_level}
                                </Badge>
                              ) : <span className="text-muted-foreground">-</span>}
                            </TableCell>
                            <TableCell className="text-center">
                            {record.prediction?.rf ? (
                                <Badge variant="outline" className="shadow-sm font-medium" style={{ color: TRAFFIC_COLORS[record.prediction.rf], borderColor: TRAFFIC_COLORS[record.prediction.rf] + '50', backgroundColor: TRAFFIC_COLORS[record.prediction.rf] + '10' }}>
                                  {record.prediction.rf}
                                </Badge>
                              ) : record.prediction?.traffic_level && record.input?.algorithm === 'rf' ? (
                                <Badge variant="outline" className="shadow-sm font-medium" style={{ color: TRAFFIC_COLORS[record.prediction.traffic_level], borderColor: TRAFFIC_COLORS[record.prediction.traffic_level] + '50', backgroundColor: TRAFFIC_COLORS[record.prediction.traffic_level] + '10' }}>
                                  {record.prediction.traffic_level}
                                </Badge>
                              ) : <span className="text-muted-foreground">-</span>}
                            </TableCell>
                            <TableCell className="text-muted-foreground whitespace-nowrap text-sm text-center">
                              <span className="px-2 py-1 rounded bg-secondary/30">{new Date(record.timestamp).toLocaleString()}</span>
                            </TableCell>
